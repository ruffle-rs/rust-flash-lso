...
use crate::types::{ClassDefinition, ECMAArrayObjectValue, Element, ObjectId, ObjectValue, Reference, Value};
use nom::Err;
...
fn parse_element_date(i: &[u8]) -> AMFResult<'_, Value> {
    ...
    Ok((i, Value::Date{ time: millis, timezone_or_utc: Some(time_zone)}))
}
...
fn parse_element_xml(i: &[u8]) -> AMFResult<'_, Value> {
    ...
    Ok((i, Value::XML {value: content.to_string(), is_string: true}))
}
...
impl AMF0Decoder {
    ...
    fn parse_element_strict_array<'a>(&mut self, i: &'a [u8]) -> AMFResult<'a, Value> {
        ...
        Ok((i, Value::StrictArray { id: ObjectId::INVALID, values: elements}))
    }
